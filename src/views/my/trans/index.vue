          ...
          <template v-if="record.length > 0">
            <ul ref="recordList">
              <li class="item" v-for="(item, index) in record" @click="goDetail(item)">
                <span class="market">
                 <strong><span v-if="item.direction===1">{{$t('exchange.exchange_buys')}}</span><span class="sell" v-else>{{$t('exchange.exchange_sells')}}</span> <template v-if="item.direction===1">{{item.toSymbol}}/{{item.fromSymbol}}</template><template v-else>{{item.fromSymbol}}/{{item.toSymbol}}</template></strong>
                  <p class="ft-c-lightGray f24">{{new Date(Number(item.createdAt)).format()}}</p>
                </span>
                <span class="price">
                  {{getPrice(item.averagePrice)}}
                  <p class="ft-c-lightGray f24"><template v-if="item.direction===1">{{item.fromSymbol}}</template><template v-else>{{item.toSymbol}}</template></p>
                </span>
                <span class="vol ellipsis">
                  {{toFixed(item.finishedAmount) | removeEndZero}}
                  <p class="ft-c-lightGray f24"><template v-if="item.direction===1">{{item.toSymbol}}</template><template v-else>{{item.fromSymbol}}</template></p>
                </span>
                <i class="allow-right"></i>
              </li>
            </ul>
            <noMoreData v-if="!recordBottomLoading && noMoreRecordHistory" :bgColor="'#F7F7F7'"/>
          </template>
...
.page-main {top: 1.9rem;}
.sepline {border-top: 0.2rem solid #f6f6f6; margin-top: 0.9rem;}
.record-box.header ul li {color: #999;  line-height: 0.5rem;}
.record-box ul li {
  padding: 0.15rem 0.3rem; line-height: 0.4rem; display: flex; border-bottom: 1px solid #ddd; position: relative;
}
.record-box ul li span {
  word-break: break-all;
  &+span {margin-left: 0.15rem;}
}
.record-box ul li span.market{width: 40%;}
.record-box ul li span.price{width: 27%;}
.record-box ul li span.vol{flex: 1; text-align: right; margin-right: 0.3rem;}
.record-box ul li span.market span{color: #01C89F; font-weight: bold; margin-right: 0.2rem;}
.record-box ul li span.market span.sell{color: #F07180;}
.allow-right:after {right: 0.3rem;}
</style>
