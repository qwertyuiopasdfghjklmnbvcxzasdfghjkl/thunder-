        ...
        <div class="page-main">
            ...
            <div class="inner_text">
                ...
                <ul>
                    ...
                    <li class="ft-c-main" v-if="symbol==='EOS' || symbol==='XRP'">3. {{$t('account.userRechargeMemoTip')}}
                        <!-- Memo和Address需要同时使用才能充值成功！ --></li>
                </ul>
            </div>
            <section class="block wallet-contant mt20">
                    <p class="title"><strong>{{symbolInfo.symbol}}{{$t('referral.address')}}</strong></p>
                    <div ref="qrcode" class="qrcode"></div>
                    <p class="two f26">{{getAddress}}</p>
                    <p class="tc mt50 pb20"><mt-button type="primary" class="mini" v-tap="{methods:copy}">{{$t('referral.copy')}}<!--复制--></mt-button></p>
                <template v-if="symbol==='EOS' || symbol==='XRP'">
                    ...
                    <p class="tc mt50 pb20"><mt-button type="primary" class="mini" v-tap="{methods:copyMemo}">{{$t('referral.copy')}}<!--复制--></mt-button></p>
                </template>
            </section>
            <div class="mt30"></div>
        </div>
