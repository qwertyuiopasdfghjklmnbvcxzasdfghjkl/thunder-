<template>
  <div class="confirm">
    <div class="title">{{mobileState === 1 ? $t('auth_warning.warning_SMS_auth') : $t('account.safety_verification')}}<!--短信验证||安全验证--></div>
    <!--短信验证-->
    <div class="form" v-if="mobileState === 1">
      <input class="input" type="password" v-validate="'required'" name="passwordVerify" :msgs="msgs.password" :errs="errors" v-model="comData.password" :title="$t('account.user_center_login_password')" :placeholder="$t('account.user_center_login_password')"><!--登录密码-->
      <div class="smsCode">
        ...
        <mt-button type="primary" style="font-size: 0.28rem;" :disabled="disabled" @click="sendSMSCode">{{$t('account.user_center_send_SMS')}}<!--发送验证码-->{{disabled ? `（${time}s）` : ''}}</mt-button>
      </div>
      <div class="buttons">
        <span class="mint-button--default" @click="closeDialog">{{$t('otc_legal.otc_legal_cancel')}}<!--取消--></span>
...
export default {
  ...
  computed: {
    ...
    msgs () {
      return {
        verifyCode: {required: this.$t('login_register.inputGoogleAuthCode'), pInteger: this.$t('error_code.NUMERIC')}, // 请输入验证码
        ...
        safetyCode: {required: this.$t('account.safety_code_required'), pInteger: this.$t('error_code.NUMERIC')} // 请输入安全验证码
      }
    }
  },
  created () {
...
.smsCode.safetyCode {margin-top: 0; margin-bottom: 0;}
.smsCode input{width: 60%;}
.smsCode span{display: inline-block;width: 2.3rem;height: 0.9rem;line-height: 0.9rem;text-align: center;cursor: pointer; border-radius: 4px;} 
.smsCode span.disabled{background:#999;}
.buttons span{display: inline-block;width: 3rem;height: 1rem;line-height: 1rem;text-align: center;cursor: pointer; font-size: 0.3rem;  border-radius: 4px;}
