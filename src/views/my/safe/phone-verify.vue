<template>
  <div class="page">
    <top-back :back="true">
      {{getUserInfo.mobileAuthEnable !== 1?$t('account.user_bind_SMS_verification'):$t('account.user_unbind_SMS_verification')}}
    </top-back>
    ...
    <div class="page-main">
      ...
      <mt-button type="primary" class="mt50" size="large" :disabled="lock" v-tap="{methods: bindMobile}">{{getUserInfo.mobileAuthEnable !== 1 ? $t('account.user_center_operate_bind') : $t('account.user_center_operate_unbind')}}<!--绑定--></mt-button>
    </div>
