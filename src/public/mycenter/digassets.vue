            ...
            <div class="chargeWithdraw" v-if="!showLoaing">
                <div class="balance_search">
                    <div class="total">
                        ...
                        {{getUSDTValuation}} USDT ≈ {{getCoinSign}} {{USDCNY}}
                    </div>
                    <div class="f-fr">
...
export default {
  ...
  computed: {
    ...mapGetters(['getBTCValuation', 'getUSDCNY', 'getCoinSign', 'getUSDTRate']),
    USDCNY () {
      return numUtils.mul(this.getBTCValuation, this.getUSDCNY).toFixed(2).toMoney()
    },
    getUSDTValuation(){
      return numUtils.mul(this.getBTCValuation, this.getUSDTRate).toFixed(4).toMoney()
    }
  },
  watch: {
