      ...
      <div class="top">
        ...
        <div class="info" :class="{active:!vsloaded}">
          ...
          <div class="info-message">
            ...
            <p class="community">
              <span>{{$t('business.ATTRIBUTION_COMMUNITY')}}：<!--归属社区--></span>
              <span class="nickname-text" v-if="isAlreadyModifyCommunity.length">{{isAlreadyModifyCommunity[0].communityName}}</span>
              <span class="nickname-modify" v-else="!isAlreadyModifyCommunity.length">
                <i v-if="!showCommunitySelect"  @click="showCommunitySelect = true">{{$t('business.CHOOSE_YOUR_COMMUNITY')}}<!--选择归属社区--></i>
                <template v-else>
                  <select name="community" class="select" v-model="community">
                    <option v-for="item of communities" :value="item.communityId">{{item.communityName}}</option>
                  </select>
                  <button type="button" class="submit" @click="changeCommunity" :disabled="locked">{{$t('otc_legal.otc_legal_confirm')}}</button><!--确定-->
                  <button type="button" class="cancel" @click="showCommunitySelect = false">{{$t('otc_legal.otc_legal_cancel')}}</button><!--取消-->
                </template>
              </span>
            </p>
            <p class="sale">
              <span>
                {{$t('account.user_center_pay_fees').format('NMS', '50%')}}<!--使用NMS支付交易手续费（50% 折扣）-->
...
export default {
  ...
  computed: {
    ...
    isAlreadyModifyCommunity(){
      let rst = this.communities.filter(item=>{
  ...
  methods: {
    changeCommunity(){
      this.locked = true
      userUtils.changeCommunity({communityId:this.community}, msg=>{
        this.locked = false
    ...
    getCommunityList(){
      userUtils.getCommunityList(res=>{
        this.communities = res
        this.community = res[0]&&res[0].communityId || ''
      })
    },
...
.submit:disabled {background-color: #eee; color: #999;}
.cancel {background-color: #eee; color: #333;}
</style>
